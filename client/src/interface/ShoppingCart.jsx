...
export const ShoppingCart = () => {
  ...
  return (
    <div className={`ShoppingCart right-0 ${shoppingCartIsOpen ? 'w-full' : 'w-fit'}`}>
      ...
      />
      <div
        className={`ShoppingCart__content fluid ${(shoppingCartIsOpen) ? '' : '-right-[30rem] sm:-right-[30rem]'}`}
      >
        ...
        {(order.total_products)
          ? <>
              <div className={`ShoppingCart__confirm ${order.total_products >= 6 ? 'discount' : ''}`}>
                <button
                  className="ShoppingCart__confirm-button fluid"
                  onClick={() => handlePayShoppingCart('/account/orders/checkout')}
                >
                  <span>Go to pay:</span>
                  <p>
                ...
                <div>
                  {(order.total_products >= 6)
                    ...
                    : <p>
                        <span>{6 - order.total_products}</span> product(s) more to{' '}
              ...
              <Link
                className="ShoppingCart__link fluid"
                to={'/products'}
                onClick={handleCloseShoppingCart}
              >
                See more products
          ...
          : <div className="ShoppingCart__empty">
              ...
              <button
                className="ShoppingCart__no-products fluid"
                onClick={() => handlePayShoppingCart('/products')}
              >
                Shop Now
              </button>
            </div>
