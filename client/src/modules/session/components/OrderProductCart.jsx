...
export const OrderProductCard = ({ item, discount }) => {
  ...
  return (
    <div className="OrderProductCard fluid">
      <img
        className="OrderProductCard__image"
        src={resize(product.image, 100)}
        alt={product.name}
      />
      <Link
        className="OrderProductCard__name fluid"
      ...
      <span className="OrderProductCard__price">
        {currencyFormatter(
          ((discount) ? product.prices.wholesale : product.prices.retail) *
          item.count
        )}
      </span>
    </div>
