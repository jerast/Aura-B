      ...
      <article className="OrderConfirm OrderConfirm--order">
            ...
            <tbody>
                ...
                <td>
                  {currencyFormatter(
                    (activeOrder.discount)
                      ? activeOrder.list.reduce(
                        (accum, item) => accum + (item.prices.retail - item.prices.wholesale) * item.count, 0
                      ) * -1
                      : 0
                  )}
                </td>
              ...
              <tr className="OrderConfirm__state">
                ...
                <td className="flex justify-end">
                  <span className={`OrderCard__state ${handleOrderStateClass()}`}>
                    {activeOrder.state}
                  </span>
                </td>
              </tr>
        ...
        <div className="OrderConfirm__list">
          {activeOrder.list.map((item) => (
            <OrderProductCard key={item.product} item={item} discount={activeOrder.discount} />
          ))}
        </div>
      </article>
    </section>
